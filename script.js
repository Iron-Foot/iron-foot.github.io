...
// Scroll-driven video for project pages
document.addEventListener('DOMContentLoaded', function() {
  const projectVideo = document.querySelector('.project-video video');
  
  if (!projectVideo) return; // Exit if no project video on page
  ...
  function setupScrollVideo() {
    ...
    function updateVideoTime() {
      ...
      const scrollProgress = Math.max(0, Math.min(1, (rect.top - scrollStart) / (scrollEnd - scrollStart)));
      
      // Invert the progress so video plays forward as we scroll down
      const videoProgress = 1 - scrollProgress;
      
      // Calculate the target time based on scroll position
      const targetTime = videoProgress * videoDuration;
      
      // Only update if the difference is significant (reduces jitter)
      if (Math.abs(projectVideo.currentTime - targetTime) > 0.1) {
        projectVideo.currentTime = targetTime;
      }
